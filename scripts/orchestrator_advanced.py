...
# -*- coding: utf-8 -*-
"""
灵犀 - 高级编排器（复杂任务三步法 S0→S1→S2→S3）
心有灵犀，一点就通

📚 参考文档：
- "这是我迄今为止开发的最满意的一个技能" - 复杂任务三步法方法论
- Complex Task Three-Step Methodology — A Layered Assessment & Execution Framework

核心优化（基于原文档）：
1. S0 零成本预筛选 - 规则匹配，0 token，过滤 ~80% 消息
2. S1 轻量复杂度评估 - 五维打分（步骤数/知识域/不确定性/失败代价/工具链）
3. S2 深度规划 & 审计 - DAG 执行蓝图，锁定机制，Plan-Audit 循环
4. S3 分阶段执行 & 质量控制 - Phase 并行，QA 审计，缺陷修改分级

设计原则（源自原文档）：
- 分层防御，而非一刀切（Defense in Depth）
- 灵敏度 - 成本的帕累托平衡
- 规划是铁约束，不是参考建议（Design Freeze）
- 并行是天然的，串行是例外（DAG 结构）
- 质量不是事后检查，而是贯穿全程
"""
